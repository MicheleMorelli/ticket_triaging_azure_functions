...
def get_new_fitted_vectoriser(dataset: List[str]) -> TfidfVectorizer:
    """
    Takes training data in form of a list of strings, and 
    returns a new fitted TD-IDF vectoriser
    """
...
def convert_pd_series_with_NaN_to_string_list(series: pd.Series) -> List[str]:
    """
    Takes a Pandas Series, and returns a list of strings. 
    This was to solve the issues with the vectoriser, 
    which does not like Pandas's NaN elements (which are of 
    type float)
    """
...
def dump_fitted_vectoriser_to_pickle(field:str,series:pd.Series)->str:
    """
    Dumps a fitted vectoriser to a pickle file. It returns 
...
def dump_vectorised_data_to_pickle(vect:TfidfVectorizer,field:str,series:pd.Series)->str:
    """
    Dumps a vectorised features to a pickle file. 
    It returns the name of the pickle file
    """
    data = convert_pd_series_with_NaN_to_string_list(series)
...
def dump_all_vectorisers_to_pickle(full_dataset:pd.DataFrame, fields_list:List[str])->None:
    """
    Dumps a pickled fitted vectoriser for each of 
    the fields in the list passed as an argument
...
def dump_all_vectorised_data_to_pickle(vect:TfidfVectorizer,full_dataset:pd.DataFrame, fields_list:List[str])->None:
    """
    Dumps a a vectorised dataset to pickle for 
    each of the fields in the list passed as an argument
    Useful when changing and testing the engineered features.
...
if __name__ == '__main__':
    """
    Dumps all vectorised features for both 
